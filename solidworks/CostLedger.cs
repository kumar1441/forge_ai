...
namespace Forge.SolidWorks
{
    ...
    /// running total so Usage Diagnostics shows total cost across sessions. Contains only counts/costs —
    ...
    internal static class CostLedger
    {
        ...
        /// <summary>Human summary for the hidden Usage Diagnostics command.</summary>
        public static string Diagnostics()
        {
            double avg = TotalOps > 0 ? TotalCostUsd / TotalOps : 0;
            string cap = UsageConfig.Enforce ? (" / " + UsageConfig.MaxOperations) : " (measure — no cap)";
            return "Usage diagnostics\n" +
                   "Ops used: " + OpCounter.Count + cap + "\n" +
                   "Total est cost this usage: $" + TotalCostUsd.ToString("F4", CultureInfo.InvariantCulture) + "\n" +
                   "Avg cost / op: $" + avg.ToString("F4", CultureInfo.InvariantCulture) + "\n" +
                   "Total tokens: " + TotalTokens + "\n" +
                   "Mode: " + UsageConfig.Mode + "  ·  Days left: " + UsageInit.DaysLeft();
        }
    }
}
