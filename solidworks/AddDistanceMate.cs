...
{
    public class AddDistanceMateResult
    {
        public string MateName;
        public string CompA, CompB;
        public double DistanceMm;
        public double AppliedMm = -1;   // read back off the mate feature-data
        public int MatesBefore, MatesAfter;
        public bool AlreadyDone;        // idempotency: a distance mate already ties these two components together
    ...
    /// <summary>
    /// Tool 54 — add_distance_mate (WRITE). Adds ONE distance mate between a planar face on each of two named
    /// components, holding them a stated distance apart. "add a 10mm distance mate between the bolt and the plate".
    /// Resolves both components (ONE question on 0/ambiguous — Rule #2), parses the distance (default mm), picks the
    /// LARGEST planar face on each, calls AssemblyDoc.AddMate5 with the distance, tags it Forge-Dist-&lt;a&gt;-&lt;b&gt;,
    /// and verifies by an INDEPENDENT re-count AND a read-back of the APPLIED distance off the mate feature-data.
    /// Idempotent — if a distance mate already ties BOTH components it does nothing. Undoable; Forge never saves.
    /// </summary>
    public static class AddDistanceMate
    {
        public static bool IsIntent(string cmd)
        {
            ...
            return Regex.IsMatch(c, @"\b(add|create|make|insert|place|put)\b") &&
                   Regex.IsMatch(c, @"\bdistance\b") && Regex.IsMatch(c, @"\bmate\b") &&
                   !Regex.IsMatch(c, @"\b(suppress|unsuppress|delete|remove|drop|info|details|describe|list|edit|change)\b");
        }

        public static async Task<AddDistanceMateResult> Run(ISldWorks app, IModelDoc2 model, string intent, Func<string, string, string, string, Task> emit)
        {
            var res = new AddDistanceMateResult();
            var asm = model as AssemblyDoc;
            if (asm == null) { res.Error = "Open the assembly (.SLDASM) to add a mate."; return res; }

            double? dmm = ParseDistanceMm(intent);
            if (dmm == null) { res.Error = "What distance? e.g. \"add a 10mm distance mate between the bolt and the plate\"."; await emit("Gauge", null, "fail", "no value"); return res; }
            res.DistanceMm = dmm.Value;
            ...
            else { res.Error = "Which two components? e.g. \"add a 10mm distance mate between the bolt and the plate\"."; await emit("Gauge", null, "fail", "need two targets"); return res; }

            try { res.CompA = compA.Name2; res.CompB = compB.Name2; } catch { }
            await emit("Gauge", "distance " + res.DistanceMm + "mm: '" + res.CompA + "' <-> '" + res.CompB + "'", "run", null);

            var mates = CollectMates(model);
            res.MatesBefore = mates.Count;

            if (MateOfTypeExists(mates, compA, compB, swMateType_e.swMateDISTANCE))
            {
                res.AlreadyDone = true; res.Verified = true; res.MatesAfter = res.MatesBefore;
                res.Info = "A distance mate already ties '" + res.CompA + "' and '" + res.CompB + "' — nothing to do.";
                await emit("Sentinel", null, "done", "already a distance mate — nothing to do");
                return res;
            }
            ...
            await emit("Torque", "adding " + res.DistanceMm + "mm distance mate", "run", null);
            ...
            double dMeters = res.DistanceMm / 1000.0;
            int err = -999; object mate = null;
            try
            {
                mate = asm.AddMate5((int)swMateType_e.swMateDISTANCE, (int)swMateAlign_e.swMateAlignCLOSEST,
                    false, dMeters, 0, 0, 0, 0, 0, 0, 0, false, false, 0, out err);
            }
            ...
            if (mate != null && err == (int)swAddMateError_e.swAddMateError_OverDefinedAssembly)
            {
                ...
                res.Error = "A distance mate between '" + res.CompA + "' and '" + res.CompB + "' would over-define the assembly — not added.";
                await emit("Sentinel", null, "fail", "would over-define");
                return res;
            }

            string tag = "Forge-Dist-" + Sanitize(res.CompA) + "-" + Sanitize(res.CompB);
            ...
            res.MatesAfter = after.Count;
            // read the applied distance back off the mate feature-data (independent of the value we passed in)
            if (mate != null) { try { var dd = ((Feature)mate).GetDefinition() as IDistanceMateFeatureData; if (dd != null) res.AppliedMm = Math.Round(dd.Distance * 1000.0, 4); } catch { } }

            bool countOk = res.MatesAfter == res.MatesBefore + 1;
            bool valueOk = res.AppliedMm < 0 || Math.Abs(res.AppliedMm - res.DistanceMm) <= 0.01;   // read-back may be unavailable
            res.Verified = countOk && valueOk;
            if (!res.Verified)
            {
                res.Error = !countOk
                    ? "Mate count didn't rise by 1 (" + res.MatesBefore + " -> " + res.MatesAfter + ", AddMate5 code=" + err + ")."
                    : "Applied distance " + res.AppliedMm + "mm != requested " + res.DistanceMm + "mm.";
                await emit("Sentinel", null, "fail", res.Error);
                return res;
            }

            string vtxt = res.AppliedMm >= 0 ? (" at " + res.AppliedMm + "mm (read back)") : "";
            await emit("Sentinel", null, "done", "distance mate added" + vtxt + " (" + res.MatesBefore + " -> " + res.MatesAfter + " mates)");
            res.Info = "Added a " + res.DistanceMm + "mm distance mate between '" + res.CompA + "' and '" + res.CompB + "'" + vtxt + ". One Ctrl+Z removes it; Forge didn't save.";
            return res;
        }
        ...
        private static double? ParseDistanceMm(string intent)
        {
            ...
            var m = Regex.Match(intent, @"(\d+(?:\.\d+)?)\s*(mm|millimet\w*|cm|centimet\w*|m\b|meter\w*|metre\w*|in\b|inch\w*|"")", RegexOptions.IgnoreCase);
            if (!m.Success)
            {
                // a bare number is taken as mm (the SW default doc unit here)
                var m2 = Regex.Match(intent, @"(\d+(?:\.\d+)?)");
            ...
            if (u.StartsWith("cm") || u.StartsWith("centi")) return v * 10.0;
            if (u == "m" || u.StartsWith("meter") || u.StartsWith("metre")) return v * 1000.0;
        ...
        {
            s = (s ?? "").Trim();
            s = Regex.Replace(s, @"\b(\d+(?:\.\d+)?)\s*(mm|millimet\w*|cm|centimet\w*|m|meter\w*|metre\w*|in|inch\w*|"")?\b", "", RegexOptions.IgnoreCase).Trim();
            s = Regex.Replace(s, @"\b(distance|apart|mates?|together|please)\b", "", RegexOptions.IgnoreCase).Trim();
