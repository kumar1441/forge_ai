...
{
    /// <summary>
    /// Per-build trial configuration. ONE place, flipped per trial build:
    ///   Trial 1 → Mode = "measure"  (learn real costs, NEVER block)
    ///   Trial 2 → Mode = "enforce"  (caps active)
    /// Expiry itself is handled dynamically by TrialInit (14 days from first run).
    /// </summary>
    internal static class TrialConfig
    {
