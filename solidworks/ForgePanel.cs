...
namespace Forge.SolidWorks
{
    ...
    public partial class ForgePanel : UserControl
    {
        ...
        private async Task Generate(string intent)
        {
            ...
            // Trial gates — run nothing if we couldn't set up safely, or the trial has ended.
            if (!TrialInit.Ready)
            ...
            { Telemetry.Log("limit_block", opsUsed: OpCounter.Count); Send(new { type = "error", message = "Trial limit reached — contact Ravi 628-363-1774" }); return; }
            ...
            if (act.Action == "ceiling")
            {
                ...
                Send(new { type = "error", message = "This assembly is too large for the trial version — contact Ravi 628-363-1774" });
                return;
            }
