...
{
    /// <summary>
    /// INDEPENDENT ground truth for CircularPatternComponent. Shares no code with the handler: re-derives the
    /// seed component from the SAME natural-language name (via SelectComponent.Normalize, the shared normalizer)
    /// but with its OWN traversal — a recursive GetRootComponent3/GetChildren tree walk instead of the handler's
    /// flat GetComponents(false) array (the same shape LinearPatternComponent's own GT uses) — and re-counts
    /// instances / over-define / rebuild / the Forge-CircularPatternComponent feature independently.
    /// </summary>
    public static partial class GroundTruth
    {
        public static JObject MeasureCircularPatternComponent(ISldWorks app, IModelDoc2 model, string intent)
        {
            var d = new JObject();
            var asm = model as AssemblyDoc;
            if (asm == null) { d["error"] = "active doc is not an assembly"; return d; }

            string query = CpcParseComponentName(intent);
            ...
            CpcWalk(root, normQuery, pathCounts, ref totalComponents, ref seedPath);
            ...
            // for the FeatureCircularPattern4-component-mode dead-API finding) — "patterned" is read the same
            ...
            d["hasForgeCircularPattern"] = hasMultipleInstances;
            d["fingerprint"] = new JObject { ["totalComponents"] = totalComponents, ["overDefinedComponents"] = overDefined };
            return d;
        }

        private static void CpcWalk(Component2 comp, string normQuery, Dictionary<string, int> pathCounts, ref int total, ref string seedPath)
        {
            if (comp == null) return;
            ...
            foreach (var o in children ?? new object[0])
            {
                ...
                CpcWalk(c, normQuery, pathCounts, ref total, ref seedPath);
            }
        }

        private static string CpcParseComponentName(string intent)
        {
            string raw = intent ?? "";
