...
namespace Forge.SolidWorks
{
    ...
    {
        // INDEPENDENT check for set_view_scale: re-reads the active drawing's own view list directly (dd.GetViews(),
        // not the handler's own Before/AfterScale report) and, for each view, its raw Name + GetOrientationName() +
        // ScaleDecimal — a completely different path than the handler's own EnumerateViews/target resolution.
        public static JObject MeasureSetViewScale(ISldWorks app)
        {
            var res = new JObject();
