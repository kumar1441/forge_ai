...
{
    public class AddCoincidentMateResult
    {
        public string MateName;
        public string CompA, CompB;
        public int MatesBefore, MatesAfter;
        public bool AlreadyDone;   // idempotency: a coincident mate already ties these two components together
    ...
    /// <summary>
    /// Tool 53 — add_coincident_mate (WRITE). Adds ONE coincident mate between a planar face on each of two named
    /// components (makes them flush/coplanar). "add a coincident mate between the bolt and the plate". Resolves both
    /// components from the live tree (ONE question on 0/ambiguous — Rule #2), picks the LARGEST planar face on each,
    /// calls AssemblyDoc.AddMate5 (the most-proven write API on this build), tags the mate Forge-Coin-&lt;a&gt;-&lt;b&gt;,
    /// and verifies by an INDEPENDENT re-count: the total mate count rose by exactly 1. Idempotent — if a coincident
    /// mate already references BOTH components it does nothing. Undoable (one Ctrl+Z); Forge never saves.
    ...
    /// </summary>
    public static class AddCoincidentMate
    {
        public static bool IsIntent(string cmd)
        {
            if (string.IsNullOrEmpty(cmd)) return false;
            string c = cmd.ToLowerInvariant();
            // NARROW + specific-first: an ADD verb + the word "coincident" (or "flush"/"coplanar"). Excludes the other
            // mate verbs so it never shadows suppress_mate/delete_mate/get_mate_info, and being distinct from
            // "concentric" keeps add_concentric_mate and the fastener auto-mate pipeline clear.
            return Regex.IsMatch(c, @"\b(add|create|make|insert|place|put)\b") &&
                   Regex.IsMatch(c, @"\b(coincident|coplanar|flush)\b") &&
                   !Regex.IsMatch(c, @"\b(suppress|unsuppress|delete|remove|drop|info|details|describe|list|edit|change|concentric)\b");
        }

        public static async Task<AddCoincidentMateResult> Run(ISldWorks app, IModelDoc2 model, string intent, Func<string, string, string, string, Task> emit)
        {
            var res = new AddCoincidentMateResult();
            ...
            else { res.Error = "Which two components? e.g. \"add a coincident mate between the bolt and the plate\"."; await emit("Gauge", null, "fail", "need two targets"); return res; }

            try { res.CompA = compA.Name2; res.CompB = compB.Name2; } catch { }
            await emit("Gauge", "coincident: '" + res.CompA + "' <-> '" + res.CompB + "'", "run", null);

            var mates = CollectMates(model);
            res.MatesBefore = mates.Count;

            if (MateOfTypeExists(mates, compA, compB, swMateType_e.swMateCOINCIDENT))
            {
                res.AlreadyDone = true; res.Verified = true; res.MatesAfter = res.MatesBefore;
                res.Info = "A coincident mate already ties '" + res.CompA + "' and '" + res.CompB + "' — nothing to do.";
                await emit("Sentinel", null, "done", "already coincident — nothing to do");
                return res;
            }

            Face2 faceA = LargestPlanarFace(compA);
            Face2 faceB = LargestPlanarFace(compB);
            if (faceA == null) { res.Error = "'" + res.CompA + "' has no planar face to mate coincident."; await emit("Gauge", null, "fail", "no planar A"); return res; }
            if (faceB == null) { res.Error = "'" + res.CompB + "' has no planar face to mate coincident."; await emit("Gauge", null, "fail", "no planar B"); return res; }

            await emit("Torque", "adding coincident mate", "run", null);
            ...
            try
            {
                mate = asm.AddMate5((int)swMateType_e.swMateCOINCIDENT, (int)swMateAlign_e.swMateAlignCLOSEST,
            ...
            if (mate != null && err == (int)swAddMateError_e.swAddMateError_OverDefinedAssembly)
            {
                ...
                res.Error = "A coincident mate between '" + res.CompA + "' and '" + res.CompB + "' would over-define the assembly — not added.";
                await emit("Sentinel", null, "fail", "would over-define");
                return res;
            }

            string tag = "Forge-Coin-" + Sanitize(res.CompA) + "-" + Sanitize(res.CompB);
            ...
            await emit("Sentinel", null, "done", "coincident mate added (" + res.MatesBefore + " -> " + res.MatesAfter + " mates)");
            res.Info = "Added a coincident mate between '" + res.CompA + "' and '" + res.CompB + "' (" + res.MatesBefore + " -> " + res.MatesAfter + " mates). One Ctrl+Z removes it; Forge didn't save.";
            return res;
        }
        ...
        {
            s = (s ?? "").Trim();
            s = Regex.Replace(s, @"\b(coincident|coplanar|flush|mates?|together|please)\b", "", RegexOptions.IgnoreCase).Trim();
